...
from sklearn.metrics import precision_score, recall_score, f1_score, accuracy_score #machen uch accurac von sklearn
...
def expand_vocabulary():
    ...
    """
    custom_tokens_file = './vocabulary_expansion.txt'
...
def model_def():
    ...
    model = AutoModelForSequenceClassification.from_pretrained("badmatr11x/distilroberta-base-offensive-hateful-speech-text-multiclassification",num_labels=len(encoder.classes_), id2label=id2label, label2id=label2id
    ).to(device)
...
def fine_tune(config=params):
    ...
    for epoch in range(config.epochs):
        ...
        for i,batch in enumerate(train_loader):  
            ...
            train_last_loss = train_batch_loss / config.batch_size #cuidado con el batch size, mejor hacer len(train_loader) ?
